...
def transition_model(corpus, page, damping_factor):
    ...
    # calculate length of links in pages
    dict_len = len(corpus.keys())
    links_len = len(corpus[page])

    if len(corpus[page]) < 1:
        # if len of links to current pages == 1 that mean it probability to loop over this two pages big
        # has no outgoing links , should return a randomly one of all possible
        for key in corpus.keys():
            output[key] = 1 / dict_len
    else:
        # there has outgoing links , calculate distribution
        random_factor = (1 - damping_factor) / dict_len
        even_factor = damping_factor / links_len

        for key in corpus.keys():
            if key not in corpus[page]:
                # randomly choose one of the links from page with equal probability.
                output[key] = random_factor
            else:
                # randomly choose one of all pages in the corpus with equal probability.
                output[key] = even_factor + random_factor
    return output
