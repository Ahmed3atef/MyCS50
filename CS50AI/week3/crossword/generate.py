...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        """
        # copy of orignal domain
        domain_copy = copy.deepcopy(self.domains)
        
        # iterate through domains copy
        for v in domain_copy:
            # getting the varible leangth
            length = v.length
            # iterate throygh words in domain
            for x in domain_copy[v]:
                if len(x) != length:
                    # if length of the word doesn't fit variable, delete it from the original domain
    ...
    def revise(self, x, y):
        ...
        # getting x and y overlapping cells, unpack cords to variaables
        ...
        if x_overlap:
            ...
            for xword in domains_copy[x]:
                ...
                for yword in self.domains[y]:
                    ...
                    if xword[x_overlap] == yword[y_overlap]:
                        matched_value = True
    ...
    def consistent(self, assignment):
        ...
        # check if there are any conflicts between neioghbouring variables
        for var in assignment:
    ...
    def select_unassigned_variable(self, assignment):
        ...
        # iterating through vars in domains
        for var in self.domains:
            # iterating through var in assignment
            if var not in assignment:
                # if var is not yet in assignment, add it to temp dict
                choice_dict[var] = self.domains[var]
                
        # make list of var sorted by number of remaining var
        sorted_list = [v for v, k in sorted(choice_dict.items(), key=lambda item:len(item[1]))]
        
        # return var with the minimum number of remaining values
    ...
    def backtrack(self, assignment):
        ...
        # selection one of unassigned var
